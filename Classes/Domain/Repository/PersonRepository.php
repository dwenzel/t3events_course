<?php
namespace Cps\DakosyReservations\Domain\Repository;
 ...
 ***************************************************************/
use Cps\DakosyReservations\Domain\Model\Dto\PersonDemand;

/**
 * The repository for Persons
 */
class PersonRepository extends \Webfox\T3events\Domain\Repository\AbstractDemandedRepository {
	/**
	...
	public function createConstraintsFromDemand(\TYPO3\CMS\Extbase\Persistence\QueryInterface $query,\Webfox\T3events\Domain\Model\Dto\DemandInterface $demand) {
		...
		$constraints = array();
		if($demand->getTypes()) {
			$personTypes = \TYPO3\CMS\Core\Utility\GeneralUtility::intExplode(',', $demand->getTypes());
