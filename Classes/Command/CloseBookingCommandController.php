<?php
namespace Cps\DakosyReservations\Command;
...
use Cps\DakosyReservations\Domain\Model\Dto\LessonDemand;
use Cps\DakosyReservations\Domain\Model\Lesson;
use Cps\DakosyReservations\Domain\Model\Person;
use Cps\DakosyReservations\Domain\Model\Reservation;
use Cps\DakosyReservations\Domain\Repository\PersonRepository;
...
class CloseBookingCommandController extends \TYPO3\CMS\Extbase\MVC\Controller\CommandController {
	...
	protected function closeReservations($dryRun) {
		...
		if (!$dryRun) {
			foreach ($reservations as $reservation) {
				...
				$reservation->setStatus(\Cps\DakosyReservations\Domain\Model\Reservation::STATUS_CLOSED);
	 ...
	 *
	 * @param \string $date A string that the strtotime(), DateTime and date_create() parser understands. Default: 'now'
	 * @return \Cps\DakosyReservations\Domain\Model\Dto\LessonDemand $lessonDemand
	 */
	protected function createDemandForLessonsWithExpiredDeadline($date = 'now') {
		/** @var \Cps\DakosyReservations\Domain\Model\Dto\LessonDemand $lessonDemand */
		$lessonDemand = $this->objectManager->get('\Cps\DakosyReservations\Domain\Model\Dto\LessonDemand');
	 ...
	 *
	 * @param \int $age Age in seconds
	 * @return \Cps\DakosyReservations\Domain\Model\Dto\ReservationDemand $reservationDemand
	 */
	protected function createInvalidReservationsDemand($age) {
		/** @var \Cps\DakosyReservations\Domain\Model\Dto\ReservationDemand $reservationDemand */
		$reservationDemand = $this->objectManager->get('Cps\\DakosyReservations\\Domain\\Model\\Dto\\ReservationDemand');
	...
	protected function createReservationDemandByLessonDeadline($date = 'now') {
		/** @var \Cps\DakosyReservations\Domain\Model\Dto\ReservationDemand $reservationDemand */
		$reservationDemand = $this->objectManager->get('Cps\\DakosyReservations\\Domain\\Model\\Dto\\ReservationDemand');
		$reservationDemand->setStatus(\Cps\DakosyReservations\Domain\Model\Reservation::STATUS_SUBMITTED);
